...
class DownloadManager(application: Application) : AndroidViewModel(application), DownloadManagerAPI {
    ...
    private val parsingExecutors = Executors.newFixedThreadPool(Config.Companion.nParsingThreads)
    ...
    override fun parseUrl(url: String) : ListenableFuture<ParsingTaskResult> {
        ...
        parsingExecutors.execute(object : Runnable {
            ...
            override fun run() {
                ...
                if (!AndroidNetworkUtils.Companion.isNetworkAvailable(getApplication())) {
                ...
                */
                ...
                parsingData.filename = MediaUtils.getUniqueFilenameAndLock(AndroidMediaUtils.Companion.getDownloadsDirectory(getApplication()).toString(), parsingData.filename?: "")
    ...
    override fun download(parsingData: ParsingData): ListenableFuture<DownloadableItemAction> {
        ...
        databaseFuture.addCallback(object : ListenableFuture.Callback<AndroidDownloadableItem> {
            ...
            override fun onSuccess(result: AndroidDownloadableItem) {
                ...
                val dirPath = AndroidMediaUtils.Companion.getDownloadsDirectory(getApplication())
    ...
    private fun isPaginationUrlTheSameOfTheOriginalUrl(url: String,
        ...
        paginationUrls.forEach(action = { p ->
            ...
            parsingData.filename = MediaUtils.getUniqueFilenameAndLock(
                AndroidMediaUtils.Companion.getDownloadsDirectory(getApplication()).toString(), parsingData.filename?: "")
