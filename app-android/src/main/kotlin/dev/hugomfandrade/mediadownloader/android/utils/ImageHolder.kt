...
class ImageHolder(private val mDir: File) {
    ...
    fun loadFromCache(imageURL: String): Bitmap? {
        ...
        return displayImage(cacheFile.absolutePath)
    }
    ...
    fun load(imageURL: String): Bitmap? {
        ...
        if (filePath == null) {
            if (DevConstants.Companion.showLog) Log.e(TAG, "failed to download file to cache dir")
            return null
        }
        ...
        if (bitmap == null) {
            if (DevConstants.Companion.showLog) Log.e(TAG, "failed to get bitmap from cache dir")
            return null
        }
    ...
    companion object {
        ...
        private val imageLoaderExecutors = Executors.newFixedThreadPool(Config.Companion.nImageLoadingThreads)
